import { initial3DRotationMatrixZValue } from "@/constants/initial3DMatricesValues";
import {
  MatrixType,
...
export const rotationMatrixZ3dAssignment: Assignment = {
  id: "rotation-z-matrix-3d",
  ...
  setup() {
    ...
    addMatrix({
      ...
      objectRefId: cube.id,
      matrixValue: initial3DRotationMatrixZValue,
    });
  },
  validate() {
    const cube = useScene3DStore.getState().cubes.find(c => c.id === "cube-1");
    if (!cube) return false;
    return cube.rotation.equals(new Vector3(0, 0, 45));
  },
};
